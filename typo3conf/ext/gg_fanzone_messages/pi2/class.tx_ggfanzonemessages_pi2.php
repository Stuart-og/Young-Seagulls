...
/**
 * Plugin 'GG Fan Zone Messages Full' for the 'gg_fanzone_messages' extension.
 *
 * @author	 <>
 * @package	TYPO3
 * @subpackage	tx_ggfanzonemessages
 */
class tx_ggfanzonemessages_pi2 extends tslib_pibase {
	var $prefixId      = 'tx_ggfanzonemessages_pi2';		// Same as class name
	var $scriptRelPath = 'pi2/class.tx_ggfanzonemessages_pi2.php';	// Path to this script relative to the extension dir.
	...
	function main($content, $conf) {
		...
		$GLOBALS['TSFE']->additionalHeaderData['css.skills'] = '<link rel="stylesheet" href="fileadmin/templates/css/fanzone-messages.css" type="text/css" media="screen" />';
		...
		$res = $GLOBALS['TYPO3_DB']->sql(TYPO3_db, 'SELECT * FROM  tx_ggfanzonemessages_messages WHERE deleted != 1 AND hidden != 1 AND response_to = "" ORDER BY uid DESC LIMIT 100');
		...
		if (mysql_num_rows($res) > 0) {
			...
			while ($row = mysql_fetch_assoc($res)) {
				...
				$comments_res = $GLOBALS['TYPO3_DB']->sql(TYPO3_db, 'SELECT * FROM  tx_ggfanzonemessages_messages WHERE deleted != 1 AND hidden != 1 AND response_to = '.$uid.' ORDER BY uid DESC LIMIT 100');
				if (mysql_num_rows($comments_res) > 0) {
					...
					while ($comments_row = mysql_fetch_assoc($comments_res)) {
						...
						$comments .= '
						<div class="gg-message-comment">
						  <h3>Comment from: '.$comments_user_name.'</h3>
						  <p>'.$comment.'</p>
						  <p class="message-time">'.$comment_interval.' ago</p>
						  <p class="tools">
				...
				<div class="message-small" id="gg-message-small-'.$uid.'" >
					...
					if ($comments) {
						$messages .= '<h4>Comments on this post:</h4>'.$comments;
					}
				$messages .= '
				</div><!--message-small-->';
				$i++;
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/gg_fanzone_messages/pi2/class.tx_ggfanzonemessages_pi2.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/gg_fanzone_messages/pi2/class.tx_ggfanzonemessages_pi2.php']);
}
