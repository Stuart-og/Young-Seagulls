...
/**
 * Plugin 'GG Meet the Team Page' for the 'gg_meet_the_team' extension.
 *
 * @author	 <>
 * @package	TYPO3
 * @subpackage	tx_ggmeettheteam
 */
class tx_ggmeettheteam_pi2 extends tslib_pibase {
	var $prefixId      = 'tx_ggmeettheteam_pi2';		// Same as class name
	var $scriptRelPath = 'pi2/class.tx_ggmeettheteam_pi2.php';	// Path to this script relative to the extension dir.
	...
	function main($content, $conf) {
		...
		$GLOBALS['TSFE']->additionalHeaderData['css.meettheteam'] = '<link rel="stylesheet" href="fileadmin/templates/css/meettheteam.css" type="text/css" media="screen" />';
		...
		while($stRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($stRes)) {
			...
			$stThumb = '/uploads/tx_ggmeettheteam/'.$stPics[0];
			$stName = $stRow['firstname'].' '.$stRow['lastname'];
			$Management .= '<div class="gg-player-card"><a href="gullys-gang/meet-the-team/?player_id='.$stUid.'"><img alt="" src="'.$stThumb.'" /></a><p>'.$stName.'</p></div>';
			$i++;
			if ($i % 2 == 0) {
        ...
        <div class="grid_2">
          ...
          <div id="gg-team-players-bg">
            <div id="gg-team-players-top">
              <h2 class="spacer">Get to know the <br /><strong>Albion Stars!</strong></h2>
            </div>
		...
		if ($_GET['player_id']) { // Player has been chosen
			...
			$res = $GLOBALS['TYPO3_DB']->sql(TYPO3_db, 'SELECT * FROM tx_ggmeettheteam_players WHERE deleted != 1 AND uid = '.$player_id.' ORDER BY number ASC');
			...
			$res = $GLOBALS['TYPO3_DB']->sql(TYPO3_db, 'SELECT * FROM tx_ggmeettheteam_players WHERE deleted != 1 AND player_type = 0 ORDER BY lastname DESC');
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			$specialSkills = $row['special'];
			$hobbies = $row['hobbies'];
			$music = $row['music'];
			$idol = $row['sporting_idol'];
		}
                  ...
                  <div>
                    <div>
                      <div id="gg-team-info">
                        ...
                        if ($dob) {
                        	$content .='<div class="gg-player-attribute"><strong>Date of birth:</strong> '.$dob.'</div>';
                        }
                        if ($nationality) {
                        	$content .='<div class="gg-player-attribute"><strong>Nationality:</strong> '.$nationality.'</div>';
                        }
                        if ($nickname) {
                        	$content .='<div class="gg-player-attribute"><strong>Nickname:</strong> '.$nickname.'</div>';
                        }
                        if ($specialSkills) {
                        	$content .='<div class="gg-player-attribute"><strong>Special Skills:</strong> '.$specialSkills.'</div>';
                        }
                        if ($hobbies) {
                        	$content .='<div class="gg-player-attribute"><strong>Hobbies:</strong> '.$hobbies.'</div>';
                        }
                        if ($music) {
                        	$content .='<div class="gg-player-attribute"><strong>Favourite Music:</strong> '.$music.'</div>';
                        }
                        if ($idol) {
                        	$content .='<div class="gg-player-attribute"><strong>Sporting Idol:</strong> '.$idol.'</div>';
                        }
                      ...
                      <div class="bottom-panel-bg">
                        <div class="bottom-panel">
                          <p class="football"><strong>Find out more...</strong></p>
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/gg_meet_the_team/pi2/class.tx_ggmeettheteam_pi2.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/gg_meet_the_team/pi2/class.tx_ggmeettheteam_pi2.php']);
}
