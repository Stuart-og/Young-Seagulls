	...
	var options = {
	    ...
	    success:    function() { 
			...
			$('form#gg-message-form-small').fadeOut('slow', function() {
				// Fade out compelte
				$("form#gg-message-form-small").html('<h2>Thanks.</h2><p>Your message has been sent to Gully. His favourites will appear on the message board shortly!</p>');
...
function report_message($uid) {
	$.ajax({
		...
		success: function(msg){
			alert("Gully has been told this message has been reported. He'll take a look and see what's wrong...");
		}
	});
}